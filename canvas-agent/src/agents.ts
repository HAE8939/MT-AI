...
export function runClaudeTurn(prompt: string, emit: AgentEmit) {
    if (!prompt.trim()) return;
    const child = spawnAgent("claude", ["-p", "--output-format", "stream-json", "--verbose", "--include-partial-messages", "--allowedTools", "mcp__mt-ai__*", prompt], ["ignore", "pipe", "pipe"], emit);
    if (!child) return;
...
class CodexAppClient {
    ...
    static async start(emit: AgentEmit) {
        ...
        await client.request("initialize", { clientInfo: { name: "canvas-agent", title: "MT-AI Agent", version: VERSION }, capabilities: { experimentalApi: true, requestAttestation: false } });
...
function codexConfig() {
    return { mcp_servers: { "mt-ai": { command: canvasAgentMcp.command, args: canvasAgentMcp.args, default_tools_approval_mode: "approve", startup_timeout_sec: 20, tool_timeout_sec: 90 } } };
}
...
async function writeAttachmentFile(item: AgentAttachment) {
    ...
    const file = path.join(os.tmpdir(), `mt-ai-${Date.now()}-${Math.random().toString(16).slice(2)}.${imageExt(meta || item.type)}`);
