...
const AGENT_CONNECT_STEPS = [
    { title: "方式一：在 Codex 中使用插件", text: "在 Codex app 安装 MT-AI 插件后，通过插件启动画布，插件会自动启动本地 Agent 并带上连接信息。" },
    { title: "方式二：直接运行 Agent", text: "不使用 Codex 插件时，在终端运行下面命令，再回到网页里连接或手动填入 Local URL 和 Connect token。", command: "npx -y @hae8939/mt-ai-agent" },
];
const AGENT_PLUGIN_REMOVE_COMMAND = "codex plugin remove mt-ai";
const AGENT_MCP_REMOVE_COMMAND = "codex mcp remove mt-ai";
...
export function CanvasLocalAgentPanel({ embedded, headless, autoConnect }: { embedded?: boolean; headless?: boolean; autoConnect?: boolean }) {
    ...
    const clientIdRef = useRef(typeof crypto === "undefined" || typeof crypto.randomUUID !== "function" ? `${Date.now()}` : crypto.randomUUID());
        ...
        <div className="thin-scrollbar min-h-0 flex-1 overflow-y-auto p-4">
            <div className="space-y-4">
                ...
                <div className="rounded-lg border px-3 py-2.5 text-xs leading-5" style={{ borderColor: theme.node.stroke, color: theme.node.muted }}>
                    ...
                    <div className="mt-1">只有安装 Codex 插件或手动添加 MCP 后，工具列表才会进入 Codex 上下文并增加 token 消耗；仅运行 `npx -y @hae8939/mt-ai-agent` 启动本地 Agent 不会安装 MCP。</div>
