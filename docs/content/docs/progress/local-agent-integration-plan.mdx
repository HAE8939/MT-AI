...
> **状态：主体已实现。** `canvas-agent/` npm 包、MCP Server（28 个工具）、SSE 画布连接与网页侧栏 Codex 对话均已落地，现状见 [Codex 能力清单](/docs/overview/codex-capabilities)与[本地 Codex 连接画布原理](/docs/overview/local-codex-canvas)。Claude Code 侧当前为 CLI Adapter，升级 Claude Agent SDK 见 [TODO](/docs/progress/todo)。本文档保留作设计记录。
...
网页侧边栏助手另走 Canvas Agent 的对话通道。Canvas Agent 使用官方 `@openai/codex` CLI 的 `codex app-server --stdio` 启动并续用同一个 thread，启动时会注入 `mt-ai` MCP 配置并把 `default_tools_approval_mode` 设为 `approve`，避免 Codex 自己的 MCP 审批卡住侧边栏；真正修改画布前由网页侧边栏做二次确认。
...
Canvas Agent 调用 Claude Code 时默认允许 `mcp__mt-ai__*`，避免 print 模式被工具审批卡住；真正修改画布仍由网页侧边栏确认。
...
1. 前端新增本地 Agent 侧边栏。
2. Canvas Agent 通过官方 `@openai/codex` CLI 的 `codex app-server --stdio` 接入 Codex，复用同一个 thread，注入 `mt-ai` MCP，并展示结构化事件流。
