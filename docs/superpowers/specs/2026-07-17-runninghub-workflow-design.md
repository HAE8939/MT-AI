...
## 2026-07-17 更新：迁移 OpenAPI v2（推翻上文「不迁移 v2」决策）

旧版 `/task/openapi/*` 未经真实验证，且官方文档已全面转向 v2，按用户要求直接迁移：

- 提交：`POST {baseUrl}/openapi/v2/run/workflow/{workflowId}`，请求体 `{ nodeInfoList }`（空数组 = 按工作流默认参数运行）。
- 查询：`POST {baseUrl}/openapi/v2/query`，请求体 `{ taskId }`；状态语义从 code 码（0/804/813/805）改为字符串 `QUEUED/RUNNING/SUCCESS/FAILED`，结果字段从 `data[].fileUrl` 改为 `results[].url`。
- 上传：`POST {baseUrl}/openapi/v2/media/upload/binary`，multipart 仅 `file` 字段；返回 `data.fileName` 继续作为 LoadImage 类节点的 fieldValue。
- 鉴权：apiKey 从请求体移至 `Authorization: Bearer` 请求头（三个端点统一）。
- 导出函数签名不变，执行器与运行对话框零改动。
- 预置内置模板「Z Image 亿级像素文生图」（workflowId `1997246493079834625`，来源用户指定的官方 API 文档页）作为 v2 链路验证用例；该工作流节点 ID 无法离线获取，模板 fields 为空 → 首跑提交空 nodeInfoList 按默认参数出图，自定义参数需导出 API JSON 后重新登记。
- 官方约束不变：工作流须先在平台手动跑通一次；结果 URL 与上传文件仅 24 小时有效。
