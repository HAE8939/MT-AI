...
- AI 创作：浏览器前端直连你配置的 OpenAI / Gemini 兼容接口，支持文生图、图生图、参考图编辑、文本问答、音频（TTS）和视频生成；Seedance 可通过火山方舟接入。
- 图片工具箱：双图滑杆对比、360° 全景查看与生成、非破坏性标注、裁剪、行列切图、局部重绘（比例选区 + 羽化 + 附加参考图）、本地插值放大、多角度生成（three.js 相机预览）。
- 专业工作流（BizyAir）：图纸渲染、双相机多角度、AI 超分（2048/4096）；任务中心支持跨路由、刷新恢复、取消、失败重试和结果写回画布。
...
- 也可以直接编辑 `web/public/config.json` 做部署级配置：预置模型渠道、默认模型、生成偏好和 WebDAV 同步参数会在启动时自动下发到所有打开该站点的设备。
- BizyAir 专业工作流（图纸渲染 / 双相机多角度 / AI 超分）需在配置页单独填入 BizyAir Base URL 和 API Key。
- 腾讯云 COS 媒体同步需在配置页填入 SecretId、SecretKey、Bucket、Region 等，并为 Bucket 配置 PUT/DELETE 的 CORS 规则。

### 连接 Codex（可选）

- Windows：运行 `setup-codex.bat` 一键安装 Codex 插件，或 `start-agent.bat` 直接启动本地 Agent 后在网页连接。
- 其余平台与手动配置见 [Codex Desktop 适配说明](docs/content/docs/setup/codex-desktop-setup.md) 和 [手动配置 Codex Desktop](docs/content/docs/setup/manual-codex-config.md)。
